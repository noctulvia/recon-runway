...
| `-s, --silent` | 静默模式，执行所有检查但不显示详细输出，最后输出 `source ${HOME}/.rrc` 命令，便于管道调用（如 `rr -s \| pbcopy`） |
| `-h, --help` | 显示帮助信息 |
...
```bash
# 静默模式，执行所有检查但不显示输出，最后输出 source 命令
rr -s
...
静默模式特点：
- 执行所有检查逻辑（网络检测、代理检测、字典检查、解析器检查）
- 不显示详细的检测输出和状态信息
- 最后输出 `source ${HOME}/.rrc` 命令
- 适用于自动化脚本集成、快速获取配置命令、管道操作和重定向
